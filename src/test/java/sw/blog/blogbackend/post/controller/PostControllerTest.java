...
public class PostControllerTest {
  ...
  void whenGetPostsWithPaging_thenReturnPagedPosts() throws Exception {
    PostListResponse post1 = PostListResponse.from(Post.builder().id(1L).title("게시글 1").content("Content 1").build());
    PostListResponse post2 = PostListResponse.from(Post.builder().id(2L).title("게시글 2").content("Content 2").build());
    List<PostListResponse> posts = Arrays.asList(post1, post2);

    when(postService.getAllPosts(0, 10)).thenReturn(posts);
